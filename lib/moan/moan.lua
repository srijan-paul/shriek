...
Moan = {
	...
	UI = {titleBoxPos = "left", messageboxPos = "bottom", imagePos = "left"}
}
...
function Moan.speak(title, messages, config)
	...
	local onstart = config.onstart or function()
	end
	local oncomplete = config.oncomplete or function()
	end
	...
	if Moan.currentMsgInstance == 1 then
		...
		typePosition = 0
		Moan.currentMessage =
				Moan.allMsgs[Moan.currentMsgInstance].messages[Moan.currentMsgKey]
	...
	if Moan.showingMessage then
		...
		if Moan.showingOptions then
			...
			for i = 1, #Moan.allMsgs[Moan.currentMsgInstance].options do
				...
				Moan.allMsgs[Moan.currentMsgInstance].options[i][1] =
						string.gsub(Moan.allMsgs[Moan.currentMsgInstance].options[i][1],
								Moan.optionCharacter .. " ", "")
			end
		...
		if string.sub(Moan.currentMessage, string.len(printedText) + 1,
				string.len(printedText) + 2) == "--" then
			Moan.paused = true
		else
			Moan.paused = false
		end
		...
		if typePosition <= string.len(Moan.currentMessage) then
			...
			if typeTimer <= 0 then
				...
				if string.sub(Moan.currentMessage, typePosition, typePosition) ~= " " and
						Typing then
...
function Moan.advanceMsg()
	...
	if Moan.showingMessage then
		...
		Moan.currentMessage =
				Moan.allMsgs[Moan.currentMsgInstance].messages[Moan.currentMsgKey] or ""
...
function Moan.draw()
	...
	if Moan.showingMessage then
		...
		if Moan.UI.titleBoxPos == "right" then
		...
		if Moan.showingOptions and Typing == false then
			for k, option in pairs(Moan.allMsgs[Moan.currentMsgInstance].options) do
				...
				love.graphics.print(option[1], textX + padding,
						optionsY + ((k - 1) * optionsSpace))
			end
		end
		...
		if Moan.showIndicator then
			if not (Moan.UI.imagePos == "right" and type(Moan.currentImage) == "userdata") then
				love.graphics.print(Moan.indicatorCharacter, boxX + boxW - (2.5 * padding),
						boxY + boxH - (padding / 2) - fontHeight)
			end
		end
	end
...
function Moan.keyreleased(key)
	...
	if key == Moan.selectButton then
		if Moan.paused then
			...
			RightSide = string.sub(Moan.currentMessage, string.len(printedText) + 3,
					string.len(Moan.currentMessage))
...
function Moan.setSpeed(speed)
	...
	elseif speed == "slow" then
		...
		assert(tonumber(speed),
				"Moan.setSpeed() - Expected number, got " .. tostring(speed))
...
function Moan.moveCamera()
	...
	if Moan.currentCamera ~= nil then
		...
		if (Moan.allMsgs[Moan.currentMsgInstance].x and
				Moan.allMsgs[Moan.currentMsgInstance].y) ~= nil then
			flux.to(Moan.currentCamera, 1, {
				x = Moan.allMsgs[Moan.currentMsgInstance].x,
				y = Moan.allMsgs[Moan.currentMsgInstance].y
			}):ease("cubicout")
		end
	end
end
...
function Moan.drawDebug()
	local log = { -- It works...
		"typing", Typing, "paused", Moan.paused, "showOptions", Moan.showingOptions,
  "indicatorTimer", indicatorTimer, "showIndicator", Moan.showIndicator,
  "printedText", printedText, "textToPrint", Moan.currentMessage,
  "currentMsgInstance", Moan.currentMsgInstance, "currentMsgKey",
  Moan.currentMsgKey, "currentOption", Moan.currentOption, "currentHeader",
  utf8.sub(Moan.currentMessage, utf8.len(printedText) + 1,
				utf8.len(printedText) + 2), "typeSpeed", Moan.typeSpeed, "typeSound",
  type(Moan.typeSound) .. " " .. tostring(Moan.typeSound), "Moan.allMsgs.len",
  #Moan.allMsgs
		-- "titleColor", unpack(Moan.allMsgs[Moan.currentMsgInstance].titleColor)
	}
	for i = 1, #log, 2 do
		love.graphics.print(tostring(log[i]) .. ":  " .. tostring(log[i + 1]), 10,
				7 * i)
	end
end
